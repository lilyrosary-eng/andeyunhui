...
//
// 根治：自动滚动与模糊判定共用同一个「钳制锚点」——锚点半高不超过「主歌词首行中心」附近。这样：
//   - 正常行：锚点落在其主歌词首行中心（约 17~35px），行为与原先几何中心接近且更规整；
//   - 长句折行 / 翻译多行：偏移量被统一钳制到首行高度内，锚点贴近视觉首行，不再被整行高度拽偏，
//     后续行（含矮折行）的间距与清晰度一致恢复。
const ANCHOR_HALF_MAX = 32; // ≈ 主歌词首行中心到行顶的距离上限；低于此半高的行按自身高度，高于则钳制
