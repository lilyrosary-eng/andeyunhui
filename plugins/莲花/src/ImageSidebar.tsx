...
export function ImageSidebar({ folders, customAlbums, loading, selectedFolder, onSelectFolder, onAddRoot, onRescan, onRenameFolder, onDeleteFolder, onDeleteAlbum, onOpenModuleSettings, searchQuery, onSearchChange }: ImageSidebarProps) {
  useLang();
          ...
          customAlbums.length > 0 && React.createElement('div', { key: 'custom-albums', className: 'mb-2' },
            ...
            customAlbums.map(album => {
              return React.createElement('div', { key: `album-${album.id}`, className: 'w-full text-left px-3 py-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 text-neutral-600 dark:text-stone-400 text-sm flex items-center gap-2' },
                ...
                React.createElement('span', { key: 'count', className: 'text-xs text-neutral-400 dark:text-stone-500' }, T('image.count', { n: album.images.length })),
