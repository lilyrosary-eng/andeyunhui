...
//!
//! 隔离：重型依赖在 feature flag 后（crawler/reverse/pentest/automation/gateway/tls-impersonate）。
//! 这些 feature **默认全开**（见 Cargo.toml `default`），即开箱即用、构建无需任何参数；
//! `--no-default-features` 可退回「仅 kernel 骨架 + commands stub」的精简构建（命令仍注册，
//! 调用返回 stub 错误），用于只想要骨架的场景。
