    ...
    pub fn to_graph(&self) -> DfaGraph {
        let nodes = self
            ...
            .map(|&id| DfaNode {
                ...
                accept: self.accept_states.contains(&id),
            })
            .collect();
        let mut edges: Vec<DfaEdge> = self
            .transitions
            .iter()
            .map(|((from, input), to)| DfaEdge {
                from: *from,
                to: *to,
                label: format!("{:02x}", input),
            })
            .collect();
        edges.sort_by_key(|e| (e.from, e.to, e.label.clone()));
        DfaGraph {
            nodes,
            edges,
