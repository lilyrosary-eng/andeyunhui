...
- **#19 reduced-motion 命中（已修复 ✅）**：打包 WebView2 下 `prefers-reduced-motion: reduce` 命中，①关掉 `.loader .petal` 的 `@keyframes bloom` 描边构建、②JS 光点入口 `if(reduce){breathing;return}` 跳过逐笔绘制 → 现象「静态完整线条、无逐笔构建」（区别于 #14 实心色块、#15 放大只莲花）。修复：4 份 wait-page 同源文件（wait-page/{light,dark}.html + public/{light,dark}.html）把 `.loader .petal` 移出 reduced-motion 降级、删 JS `if(reduce)return`；背景装饰（状态点/玫瑰窗旋转/门廊小玫瑰窗/烛光/星点）仍降级，中央莲花构建动画+流光作为核心加载反馈始终播放。改完重跑 `node scripts/gen-waiting-pages.mjs` 刷新 base64。验证须 `build.bat` 出 exe（dev 不表现 reduced-motion 差异、旧 exe 不含修复）。
