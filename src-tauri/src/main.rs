...
fn main() {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // ============ P0-1 实验探针（ANDY_UDF_PROBE=1）============
            // 验证「共享 WebContext」时创建第二/第三 CoreWebView2Controller 是否触发
            // 0x8007139F（#5356）：启动 6s 后懒建歌词窗，再 6s 后懒建录屏区域选择窗。
            // 配合 ANDY_WEBVIEW_UDF=shared 使用；无探针环境不产生任何行为变化。
            if std::env::var("ANDY_UDF_PROBE").as_deref() == Ok("1") {
                let h = app.handle().clone();
                tauri::async_runtime::spawn(async move {
                    tokio::time::sleep(std::time::Duration::from_secs(6)).await;
                    let r1 = lyrics_service::show_lyrics_widget(h.clone()).await;
                    eprintln!("[UDF-PROBE] lyrics: {:?}", r1);
                    tokio::time::sleep(std::time::Duration::from_secs(6)).await;
                    let r2 = recording_service::show_recorder_select(h).await;
                    eprintln!("[UDF-PROBE] recorder-select: {:?}", r2);
                });
            }
