...
export function GlobalSettingsPanel() {
  ...
  // ---- 彻底删除模块（连同文件）：复用后端 delete_plugin（api.deletePlugin 已封装）----
  const [delTarget, setDelTarget] = useState<string>(NIAOLUO_MODULES[0].id);
  const [delMsg, setDelMsg] = useState<string>('');
  const handleNiaoluoDelete = useCallback(async () => {
    ...
    try {
      ...
      hot?.unload?.(delTarget);
      await api.deletePlugin(delTarget);
            ...
            <div className="space-y-6">
              ...
              <section>
                ...
                <div className="bg-white dark:bg-stone-800/70 backdrop-blur rounded-xl border border-white/80 dark:border-stone-700/50 divide-y divide-neutral-200/50 dark:divide-stone-700/50 overflow-hidden">
                  ...
                  <div className="flex justify-between items-center p-4 gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <select
                        ...
                        className="px-2 py-1.5 rounded-lg border border-neutral-200/50 dark:border-stone-600/50 text-xs bg-white dark:bg-stone-700 text-neutral-700 dark:text-stone-300 outline-none focus:ring-2 focus:ring-[var(--element-border)]"
                      >
                        {NIAOLUO_MODULES.map((m) => (
                  ...
                  {delMsg && (
                    <div className="px-4 pb-3 text-xs text-neutral-500 dark:text-stone-400">{delMsg}</div>
                  )}
                </div>
              </section>
            </div>
