...
import { injectMarkdownStyles, attachMarkdownCopyHandler, renderMarkdown, escapeHtml } from '../../../lib/markdown';

// 向后兼容：旧 API 名 `renderMd`（测试曾引用）。
...
// 代码块复制按钮：全局事件委托（由共享 markdown 工具 attachMarkdownCopyHandler 提供）。
...
export function MessageList({ timeline, busy, cloudAvailable, onRetry, onSwitchCloud }: MessageListProps) {
  ...
  return (
    ...
    >
      {/* markdown 样式由共享工具 injectMarkdownStyles 幂等注入到 head（mount 时调用） */}
