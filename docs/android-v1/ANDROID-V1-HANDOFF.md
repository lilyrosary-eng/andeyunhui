...
> **本章是执行方（editor）的作战地图。** 每次开工前读本章确认当前位置，每次收工后回写状态列。
...
| 2026-08-02 | §10.3 T01 | 文档未提及 IDE sandbox 对 Gradle/Kotlin/AAPT2 daemon 的影响 | IDE sandbox 阻止 kotlin daemon 写 `%LOCALAPPDATA%\kotlin\daemon\`（AccessDeniedException），AAPT2 daemon 管道通信也可能被拦截 | `gradle.properties` 新增 `kotlin.compiler.executionStrategy=in-process` / `android.aapt2.useDaemon=false` / `org.gradle.daemon=false`，全部走非 daemon 模式规避 sandbox 限制。详见 `gen/android/README.md`。 |
